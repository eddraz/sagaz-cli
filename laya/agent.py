...
class Agent:
    ...
    def __init__(
        ...
        candidate_dir = None
        short_name = subfolder if subfolder else model_id_or_path.split("/")[-1]
        if short_name in ("laya", "default"):
            short_name = "english"

        # Check candidate locations under ~/models/laya and ~/models first
        for search_base in (models_base_dir, os.path.expanduser("~/models")):
            cand = os.path.join(search_base, short_name)
            if os.path.isdir(cand) and os.path.exists(os.path.join(cand, "rl_agent_config.json")) and os.path.exists(os.path.join(cand, "model.safetensors")):
                candidate_dir = cand
                break
        ...
        if candidate_dir:
            ...
            if subfolder:
                model_dir = snapshot_download(model_id_or_path, local_dir=models_base_dir, **kw)
            else:
                target_dest = os.path.join(models_base_dir, short_name)
                model_dir = snapshot_download(model_id_or_path, local_dir=target_dest, **kw)
